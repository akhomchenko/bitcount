...
Repository-specific plan guidance lives in [doc/plans/README.md](doc/plans/README.md).

- Track work in `doc/plans/<feature>/PLAN.md`.
...
- Treat `doc/plans/<feature>/PLAN.md` as the only live planning document for that work.
...
- In each atomic implementation commit, update `doc/plans/<feature>/PLAN.md` so it is clear which task or finding that commit completed.
